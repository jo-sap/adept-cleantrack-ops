...
/** ISO date strings (YYYY-MM-DD). Add or replace for your region. */
...
let phSet: Set<string> | null = null;

function getPHSet(): Set<string> {
  if (phSet) return phSet;
  phSet = new Set(PUBLIC_HOLIDAYS);
  return phSet;
}
...
export function isPublicHoliday(date: Date): boolean {
  return getPHSet().has(toKey(date));
}

/** Returns the set of public holiday date keys (YYYY-MM-DD) within the range (inclusive). */
export function getPublicHolidaysInRange(startDate: Date, endDate: Date): Set<string> {
